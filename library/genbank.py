### Functions required minor modification of those found in IVY.  
### The use of this script still requires the installation of IVY. 
...
def fetch_DNA_seqs(terms, maxn=10000, batchsize=1000):
    ...
    assert email, "set email!"
    Entrez.email = email
    h = Entrez.esearch(db="nucleotide", term=terms, usehistory="n")
    d = Entrez.read(h)
...
def fetch_genelist(gilist, batchsize=1000):
    global email
    assert email, "set email!"
    Entrez.email = email
    results = {}
