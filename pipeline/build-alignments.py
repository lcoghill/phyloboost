...
# alignment columns that have a density <  density will be removed from the alignments. 
# To ignore, set to 0. 
fasta_dir = ""      # directory location of fasta files generated by get-clusters.py
max_iters = 1       # max iterations for muscle
max_trees = 2       # max trees for muscle
alignment_dir = ""  # directory to save the alignment files
red_alignment_dir = "" # directory to save reduced alignment files
...
done_file_names = [] 
done_files = glob.glob(alignment_dir + "*.fas")

for d in done_files:
    search_str = alignment_dir + "(.+?).fas"
    f_name = re.search(search_str, d).group(1)
    done_file_names.append(f_name)
...
for f in fasta_files:
    ...
    if fasta_name not in done_file_names:
        try:
            ...
            clean_file_id = re.search(id_search, f).group(1)
            reduced_alignment_file = "".join([red_alignment_dir,clean_file_id,str(density)[1:],"_reduced.fas"])
            SeqIO.write(record_list, reduced_alignment_file, "fasta") # write the good seq record list to the same file we started with
        ...
        print "Alignment for %s already built. Skipping ahead." %f
## create stats label
