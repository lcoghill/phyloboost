...
density = 0.3                         # filter density for alignments. alignment columns that have a density less than this value will be removed from the alignments. To ignore, set to 0. 
fasta.files = "/full/path/here/"      # directory location of fasta files generated by get-clusters.py
max.iters = 2                         # max iterations for muscle
max.trees = 1                         # max trees for muscle
alignment.files = "/full/path/here/"  # directory to save the alignment files in
...
fasta_files = glob.glob("".join([fasta.files, "*.fasta"])) # get a list of all fasta files in /fasta
...
for f in fasta_files:
    ...
    muscle_cline = MuscleCommandline(input=f, maxiters=max.iters, maxtrees=max.trees)
    ...
    alignment_file = "".join([alignment.files,clean_file_id,".phy"])
    SeqIO.write(align, alignment_file, "relaxed-phylip") #after done with all iterations, write the good seq record list to the same file we started with
    
    ## write list of seq_records to fasta file
    clean_file_id = re.search('fasta/(.+?).fasta', f).group(1)
    reduced_alignment_file = "".join([alignment.files,clean_file_id,".den_",str(density),".reduced.phy"])
    SeqIO.write(record_list, alignment.files, "phylip-relaxed") #after done with all iterations, write the good seq record list to the same file we started with
